...
impl InputTest {
    ...
    pub fn render(&mut self) {
        ...
        draw_text("Input Test", left, 60.0, 32.0, TEXT);
        ...
        match &self.state {
            ...
            InputState::Running(recorder, _) => {
                let latency_ms = recorder.estimate_latency() * 1000.0;
                draw_text(&format!("Est. latency: {latency_ms:.1} ms"), left, 60.0, 18.0, TEXT_DIM);
            }
        }

        let y_btn = screen_height() - 60.0;
        if draw_button_colored(left, y_btn, 100.0, 36.0, if self.active { "Pause" } else { "Resume" }, 18.0,
        ...
        let meter_x = left + 60.0;
        let meter_w = 40.0;
        let meter_h = 300.0;
        let meter_y = 100.0;
        let gap = 80.0;

        draw_text("L", meter_x + 12.0, meter_y - 8.0, 18.0, TEXT_DIM);
        draw_meter(meter_x, meter_y, meter_w, meter_h, self.peak_l, self.hold_l);

        draw_text("R", meter_x + gap + 12.0, meter_y - 8.0, 18.0, TEXT_DIM);
        ...
        draw_text(&format!("L: {db_l:.1} dB"), meter_x, meter_y + meter_h + 16.0, 16.0, TEXT);
        draw_text(&format!("R: {db_r:.1} dB"), meter_x + gap, meter_y + meter_h + 16.0, 16.0, TEXT);
    }
}
